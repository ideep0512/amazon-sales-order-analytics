...
def main():
    ...
    final_sales_df = final_sales_df.select(
        ...
        # USD2FR exchange rate was not available in data, so USD2UK used.
        col('TAX_AMOUNT').alias('local_tax_amt'),
        col('USD2UK').alias("Exhchange_Rate"),
        (col('FINAL_ORDER_AMOUNT')/col('USD2UK')).alias('US_TOTAL_ORDER_AMT'),
        (col('TAX_AMOUNT')/col('USD2UK')).alias('USD_TAX_AMT'),
        col('payment_status'),
        col('shipping_status'),
        col('payment_method'),
    ...
    logging.info("Saving final transformed data to sales_dwh.curated.fr_sales_order...")
    #final_sales_df.show(5)
